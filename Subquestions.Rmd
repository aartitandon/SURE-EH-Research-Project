...
#reason why heat map is not working?
# y$date <- format(as.Date(y$session_start), "%Y-%m")
# str(y$date)

#heatmap attempt 1
#(x=sessionid, y=num_occurance)
ggplot(y, aes(x=sessionid, y=num_occurance)) + 
  geom_tile(colour = "white") + 
  scale_fill_gradient(low="red", high="darkgreen", name="=Legend")

# heatmap attempt 2 (does not work, come back to later)
# https://stackoverflow.com/questions/60644031/time-series-heat-map
ggplot(y, aes(x = date, y = num_occurance, fill = sessiontime))+
  geom_tile()+
...
```{r}
# heatmap attempt 5 (WORKS!)
# x=week of the year, y=month, fill="activity"/number of occurances
ggplot(y, aes(x=week, y=month)) + geom_tile(aes(fill=num_occurance)) + 
...
```{r}
# heatmap attempt 6 
# #... how to do with sessiontime?
# Hi Eddie, I am struggling to remove seconds from the 'sessiontime' column in the 'y' dataframe... do you have any suggestions? 
```
