...
-   [Looking at New Data](#looking-at-new-data)
    ...
    -   [Top 5 Performing Months on the Application from August 2020 to March 2022](#top-5-performing-months-on-the-application-from-august-2020-to-march-2022)
-   [Suggestions for Future Work](#suggestions-for-future-work)  
-   [What's next for my research and for PLN?](#whats-next-for-my-research-and-for-pln)  
-   [Appendix](#appendix)   
<br>
...
```{r chloropeth-map-wa-allcounties, echo=F, warning=F, message=F, eval=F, out.width = "100%"}
...
# Trial attempt 
ggplot() + 
  geom_polygon(data = states_sf, aes(x = long, y = lat, group = group, fill = Count)) +
  coord_fixed(1.3)
...
# Frequency of Different Languages by Device w/ Percentage Labels (N = 3671 ____e.g. activity)
...
```
<figcaption align = "center"><b>Figure 5 - Bar Chart of Language by Operating System </b></figcaption>
...
Growing season starts around mid-March till the end of November. This figure depicts the increase in session activity (i.e. clicks) mid-year. It is also important to note that pesticide training occurs from January to March and the application's peak activity occurs in February. The rise in activity at this time conveys the success of the application targeting this specific population of farm workers. 
...
In regards to the 'Day of the Month', the usage remains relatively consistent. However, there is a spike in usage of the application each day from 12-3 PM. The rise in activity (i.e. clicks) at this time conveys farm workers are looking at this information after work. A future update would be to introduce notifications on the application to remind farm workers to review safety information prior to their working day. 
...
p <-p + labs(title= paste("2020-2021 Session Analytics"), x="Day of the Month", y="Hour Commencing")
...
ggplot(download_data, aes(x = reorder(n, -per), y=per)) + 
  ...
  geom_text(aes(label=per_label), vjust=-0.25) + 
  labs(x = "Source Page", y = "Percentage") + 
  scale_y_continuous(labels(scales::percent)) + 
  theme_bw() + 
...
Amongt 129 total pesticides in the application, Table 1 displays the top ten searched pesticides. 'Lorsban® Advanced' was the most popularly looked at pesticide among users from August 2020-2021.
...
```{r updated-merged-monthly-csvs, warning=F, message=F, echo=F, results=F}
# updated.csv dataframe 

temp <- list.files(path='Updated_CSV', pattern="*.csv", full.names = T)
list2env(lapply(setNames(temp, make.names(gsub("*.csv$", "", temp))), read.csv), envir = .GlobalEnv)
rm(Updated_CSV.qAnswers)

updated.csv <- do.call(rbind, lapply(ls(pattern = "CSV"), get))
ignores <- read.csv("ignores.csv")
updated.csv <- filter(updated.csv,!(aid %in% ignores$aid))
```


```{r updated-data-sessions, warning=F, message=F, echo=F, results=F}

# creating sessions for updated.csv dataframe
library("dplyr")
library("lubridate")

#filter updated.csv from aid's first "evStart" to the next "evStart"
updated.csv.dedup <- unique(updated.csv)

updated_sessions <- updated.csv.dedup %>% 
  group_by(sessionid = cumsum(evType == 'evStart')) %>% 
  mutate(sessioneventno = row_number())
  
updated_sessions$ts <- ymd_hms(updated_sessions$ts)
updated_sessions$ts <- force_tz(updated_sessions$ts, "UTC")

setDT(updated_sessions)
updated_sessions <- updated_sessions[ , .(session_start = min(ts), session_end = max(ts), num_occurance = .N), by = sessionid]

#'session time' column is added 
updated_sessions$session_start <- with_tz(updated_sessions$session_start, tzone = "America/Los_Angeles")
updated_sessions$session_end <- with_tz(updated_sessions$session_end, tzone = "America/Los_Angeles")

updated_sessions$sessiontime <- ymd_hms(updated_sessions$session_end) - ymd_hms(updated_sessions$session_start)
updated_sessions$sessiontime2 <- as.double(updated_sessions$sessiontime)
updated_sessions$sessiontimemin <- updated_sessions$sessiontime2/60

updated_sessions$hour <- hour(updated_sessions$session_start)
updated_sessions$day <- day(updated_sessions$session_start)
updated_sessions$week <- week(updated_sessions$session_start)
updated_sessions$month <- month(updated_sessions$session_start)
updated_sessions$month <- month.abb[as.numeric(updated_sessions$month)]
# (the line above converts month number to month name)
updated_sessions$year <- year(updated_sessions$session_start)
updated_sessions$month = factor(updated_sessions$month, levels = month.abb)
updated_sessions$Month_Yr <- format(as.Date(updated_sessions$session_start), "%Y-%m")

```

## Number of Sessions Each Month 
```{r updated-data-sessions-each-month-plot, warning=F, message=F, echo=F, results="hide", eval=T}

#Number of sessions each month - plot

library(ggplot2)
library(tidyverse)
library(dplyr)

monthly_sessions <- updated_sessions %>% select(sessionid, Month_Yr)

ggplot(monthly_sessions, aes(Month_Yr, ..count..)) +
  geom_bar(aes(fill = sessionid), position = "dodge", fill="black") +
  geom_bar() +
  geom_text(aes(label = after_stat(count)), stat = "count", hjust = -0.3) +
  xlab("Year & Month") + 
...
## Comparing User Sessions by 'Hour of the Month', 'Day of the Month', 'Month', and 'Year'
```{r updated-data-heatmap, warning=F, message=F, echo=F, results=F, eval=T}
...
updated_df <-updated_sessions %>% select(sessionid,day,hour,month,year)
z <-ggplot(updated_df,aes(day,hour))+
  geom_tile(color= "white",size=0.1) + 
  scale_fill_viridis(name="Hrly Temps C",option ="C")
z <-z + facet_grid(year~month)
z <-z + scale_y_continuous(trans = "reverse", breaks = unique(df$hour))
z <-z + scale_x_continuous(breaks =c(1,10,20,31))
z <-z + theme_minimal(base_size = 8)
z <-z + labs(title= paste("2020-2022 Session Analytics"), x="Day of the Month", y="Hour Commencing")
...
```{r updated-data-sessions-each-month-table, warning=F, message=F, echo=F, results=T, eval=T}

#Number of sessions each month - table

monthly_sessions <- updated_sessions %>% select(sessionid, Month_Yr)
monthly_sessions_table <- table(monthly_sessions$Month_Yr)
monthly_sessions_table_final <- monthly_sessions_table[order(monthly_sessions_table, decreasing = TRUE)]
monthly_sessions_table_final <- monthly_sessions_table_final[1:5]
kable(monthly_sessions_table_final)

```

# Suggestions for Future Work 
In this research study, I found the biggest limitation to be the manual analysis of data. For future work, I would suggest taking advantage of R-Studio and automate the analysis process. This will allow future data to be seamlessly included in the analysis reports as needed. 

# What's next for my research and for PLN?
With a total of 503 unique users, 1328 sessions, and over 67,000 interactions (i.e. clicks) within a span of one year, the application’s success speaks for itself. Its increasing user activity each month, especially during growing season, conveys the success of the application targeting this specific population of farmworkers. However, the ‘Pesticides Labels Now!’ application is not quite done making their impact on the agricultural community. The team continues to work on delivering up-to-date safety information, an expanding pesticide list, and a better user experience till this status quo is relinquished. 
