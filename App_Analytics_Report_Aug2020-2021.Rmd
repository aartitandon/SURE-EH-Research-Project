...
chloropeth <- ggplot()
chloropeth <-chloropeth + geom_polygon(data=pln_sf_t, aes(x=long, y=lat, group=group))
chloropeth                                
...
```{r updated-data-sessions-each-month-plot, warning=F, message=F, echo=F, results="hide", eval=F}
...
monthly_sessions_table <- table[order(monthly_sessions_table, decreasing = TRUE)]
monthly_sessions_table <- monthly_sessions_table [1:5]
kable(monthly_sessions_table)
