...
###########################################################
# 1 a python program to grab the date of first use for each
#   aid in a file of all log records in JSON format
# 2 ignores aids in the ignores.csv file
# 3 output is in CSV format in this case a timestamp only 
#   per line
...
def countFirstUses(logs, ignores):
    ids = []
    dts = []
    for log in logs:
        if (log['aid'] not in ignores) and (log['evType'] == 'evStart') and (log['aid'] not in ids):
            ids.append(log['aid'])
            dts.append(log['ts'])
    print('"timestamp"')
