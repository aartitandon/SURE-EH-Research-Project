...
```{r}
# Table 2a. Percent of ¡Etiquetas de pesticidas, ahora!™/Pesticide Labels, Now!™ sessions by language.
install.packages("dplyr")
library(dplyr)
install.packages("stringr")
library(stringr)

uniquesession_evstart <- filter(x, str_detect(evType, 'evStart')) %>% 
  distinct_at(vars(sessionid), .keep_all = TRUE) 
# here we get 1327 sessions

uniquesession_vp <- filter(x, (str_detect(evType, 'evViewPage'))) %>% 
  distinct_at(vars(sessionid), .keep_all = TRUE) 
# here we get 1325 sessions because there is not 1327 evViewPage records displaying the language
table(uniquesession_vp$evDesc2)
...
```{r}
# Table 7a Alternate. Who is more likely to use the language toggle, primary Spanish or English speakers?  
install.packages("dplyr")
library(dplyr)
install.packages("stringr")
library(stringr)

uniquesession_lt <- filter(x, (str_detect(evType, 'evLanguageToggle'))) %>% 
  distinct_at(vars(sessionid), .keep_all = TRUE) 
table(uniquesession_lt$evDesc2)
...
```{r}
# how to count all values in a certain column 

```

```{r freq-of-lang-by-device, warning=F, message=F, echo=F, results=F, eval=T}
